...
// Helper function to compute epipolar line from relative pose and point
template <typename T>
void ComputeEpipolarLine(const T* rel_t, const T* rel_q, const T* p_prev_norm, T* line) {
  // Compute [t]_x (skew-symmetric matrix)
  T tx[9] = {
      T(0), -rel_t[2], rel_t[1],
      rel_t[2], T(0), -rel_t[0],
      -rel_t[1], rel_t[0], T(0)
  ...
  T E[9] = {T(0)}; // Initialize with T(0) to match the template type
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      for (int k = 0; k < 3; ++k) {
  ...
  // Compute epipolar line: line = E * p_prev_norm
